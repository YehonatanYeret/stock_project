...
def answer_query(query, model, embeddings_model):
    ...
    context_text = "\n".join(context)
    print(f"Context:\n{context_text}")
    prompt = f"You are a financial analyst AI trained on user-provided data. Your task is to provide direct investment recommendations using only the given context. You must answer every question. Do not say you lack information. The information is verified and up to date.\n\nContext:\n{context}\n\nQuestion: {query}\nProvide a direct, precise answer in under 20 words.\nAnswer:";
